...
/**
 * Harvest It form handler.
 *
 * @package DFE News Harvester
 * @subpackage Harvest_This
 * @since 2.0
 *
 * @return int Post ID
 */
function harvest_it() {
	...
	// set News Harvester meta information, if it exists
	if($_REQUEST['subtitle']) { $meta['subtitle'] = $_REQUEST['subtitle']; }
	if($_REQUEST['link_url']) { $meta['link_url'] = $_REQUEST['link_url']; }
	if($_REQUEST['link_text']) { $meta['link_text'] = $_REQUEST['link_text']; }
	if($_REQUEST['art_image']) { $meta['art_image'] = $_REQUEST['art_image']; }
	if($_REQUEST['image_credit']) { $meta['image_credit'] = $_REQUEST['image_credit']; }
	if($_REQUEST['image_cred_url']) { $meta['image_cred_url'] = $_REQUEST['image_cred_url']; }
	if($_REQUEST['subtitle']) { $meta['subtitle'] = $_REQUEST['subtitle']; }
	...
	// error handling for media_sideload
		...
		foreach($meta as $key=>$value) {
			if(!update_post_meta($post_ID, $key, $value)) { add_post_meta($post_ID, $key, $value, true); }
		}
		$nh->titleToTags($quick['ID'], $quick['post_title']);
		$nh->titleToTags($quick['ID'], $meta['link_text']);
	}
	return $post_ID;
}
...
<head>
	...
	<title><?php _e('Harvest This') ?></title>
...
<div id="poststuff" class="metabox-holder">
	<div id="side-info-column">
		<div class="sleeve">
			<h1 id="viewsite"><a class="button" href="<?php echo get_option('home'); ?>/" target="_blank"><?php bloginfo('name'); ?> &rsaquo; <?php _e('Harvest This') ?></a></span></h1>
...
<img alt="<?php _e('Insert an Image'); ?>" src="images/media-button-image.gif"/></a>
			...
			<div class="editor-container">
				<textarea name="content" id="content" style="width:100%;" class="mceEditor" rows="8">
					<?php if ($url) { echo '<p>'; if($selection) _e('via '); echo "<a href='$url'>$title</a>."; echo '</p>'; } ?>
					<?php if ($selection) echo '<blockquote>'.wp_richedit_pre(htmlspecialchars_decode($selection).'</blockquote>'); ?>
				</textarea>
			</div>
			<table>
				<tr>
					<td width="150"><label for="link_text">Teaser Title:</label></td>
					<td><input type="text" name="link_text" size="50" class="tbtitle" <?php if($url) { ?>value="<?php echo $title ?>"<?php } ?> /></td>
				</tr>
				<tr>
					<td><label for="link_url">Link:</label></td>
					<td><input type="text" name="link_url" size="50" class="tbtitle" <?php if($url) { ?>value="<?php echo $url ?>"<?php } ?> /></td>
				</tr>
				<tr>
					<td><label for="art_image">Image:</label></td>
					<td><input type="text" name="art_image" size="50" class="tbtitle" /></td>
				</tr>
				<tr>
					<td><label for="image_credit">Credit:</label></td>
					<td><input type="text" name="image_credit" size="50" class="tbtitle" /></td>
				</tr>
				<tr>
					<td><label for="image_cred_url">Credit Link:</td>
					<td><input type="text" name="image_cred_url" size="50" class="tbtitle" /></td>
				</tr>
				<tr>
					<td colspan="2"><label for="subtitle">Excerpt:</label></td>
				</tr>
				<tr>
					<td colspan="2">
						<div class="editor-container">
							<textarea name="subtitle" size="40" class="mceEditor" />
								<?php if($selection) { echo htmlspecialchars_decode($selection); } ?>
							</textarea>
						</div>
					</td>
				</tr>
